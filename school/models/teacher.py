...
class SchoolTeacher(models.Model):
    ...
    def create(self, vals_list):
        ...
        teacher_id = super(SchoolTeacher, self).create(vals_list)
        user_obj = self.env["res.users"]
        emp_group = self.env.ref("base.group_portal")
        user_vals = {
        ...
        if teacher_id.user_id:
            ...
            group_list = [teacher_group.id, emp_group.id]
            teacher_id.user_id.write({"groups_id": [(6, 0, group_list)]})
        return teacher_id
    ...
    def _check_email(self):
        for rec in self:
            if rec.work_email:
                if not re.match(r"[^@]+@[^@]+\.[^@]+", rec.work_email):
